 ...
 *
 */
public class GteParamFilter implements ParamFilter {
	...
	public void doFilter(Filter filter, Map<String, ?> params) {
		List<Gte> gtes = filter.getGtes();
		if (CollectionUtils.isEmpty(gtes)) {
			return;
		}
		for (Iterator<Gte> it = gtes.iterator(); it.hasNext();) {
			Gte gte = it.next();
			String paramsConfig = gte.getParams();
			String value = gte.getValue();
	...
	private boolean isFiltered(Object paramValue, String value) {
		if (paramValue != null) {
			if (paramValue instanceof String) {
				...
				return isDateFiltered(paramValue, value);
			} else if (value.compareTo(paramValue.toString()) <= 0) {
				return true;
			}
		}
		return false;
