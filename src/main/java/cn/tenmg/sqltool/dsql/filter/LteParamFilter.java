 ...
 *
 */
public class LteParamFilter implements ParamFilter {
	...
	public void doFilter(Filter filter, Map<String, ?> params) {
		List<Lte> ltes = filter.getLtes();
		if (CollectionUtils.isEmpty(ltes)) {
			return;
		}
		for (Iterator<Lte> it = ltes.iterator(); it.hasNext();) {
			Lte lte = it.next();
			String paramsConfig = lte.getParams();
			String value = lte.getValue();
