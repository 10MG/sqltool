...
public class OracleTest {
	...
	public static void doTest(Dao dao) {
		...
		page = dao.page(StaffInfo.class, "oracle_offset_fetch", currentPage, pageSize, params);
		Assert.assertEquals(currentPage, page.getCurrentPage());
		Assert.assertEquals(pageSize, page.getPageSize());
		Assert.assertTrue(pageSize >= page.getTotal().intValue());
		...
		page = dao.page(StaffInfo.class, "oracle_offset", currentPage, pageSize, params);
		Assert.assertEquals(currentPage, page.getCurrentPage());
		Assert.assertEquals(pageSize, page.getPageSize());
		Assert.assertTrue(page.getTotal().intValue() == countLike1 - (int) params.get("offset"));

		page = dao.page(StaffInfo.class,
				"SELECT staff_id, staff_name FROM STAFF_INFO WHERE STAFF_NAME LIKE :staffName ORDER BY STAFF_NAME FETCH NEXT :offset ROW ONLY",
				currentPage, pageSize, params);
		Assert.assertEquals(currentPage, page.getCurrentPage());
		Assert.assertEquals(pageSize, page.getPageSize());
		Assert.assertTrue(pageSize >= page.getTotal().intValue());
		...
		page = dao.page(StaffInfo.class, "oracle_fetch", currentPage, pageSize, params);
		Assert.assertEquals(currentPage, page.getCurrentPage());
		Assert.assertEquals(pageSize, page.getPageSize());
		Assert.assertTrue(pageSize >= page.getTotal().intValue());
	}
